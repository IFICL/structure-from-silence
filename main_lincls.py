...
def train(args, device):
    ...
    # -------- Loading trained feature weights ------------- #
    pertrained_model = models.__dict__[pr.load_model](args, pr)
    if args.resume:
        resume = './checkpoints/' + args.resume
        pertrained_model, _ = torch_utils.load_model(resume, pertrained_model, strict=True)
    # ------------------- #

    # ----- Network ----- #
    net = models.__dict__[pr.net](args, pr, model=pertrained_model, device=device).to(device)
    ...
    writer.add_scalars('SFS-LinCLS' + '/validation', res, args.start_epoch)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training"):
            ...
            if (step+1) % BOARD_STEP == 0:
                writer.add_scalar('SFS-LinCLS' + '/training loss', running_loss / BOARD_STEP, current_step)
                running_loss = 0.0
            
            if (current_step + 1) % VALID_STEP == 0 and args.valid_by_step:
                res = validation(args, pr, net, criterion, val_loader, device)
                writer.add_scalars('SFS-LinCLS' + '/validation-on-Step', res, current_step)
        ...
        if (epoch + 1) % VALID_STEP == 0 and not args.valid_by_step:
            res = validation(args, pr, net, criterion, val_loader, device)
            writer.add_scalars('SFS-LinCLS' + '/validation', res, epoch + 1)
...
def test(args, device):
    ...
    # ----- Network ----- #
    pertrained_model = models.__dict__[pr.load_model](args, pr)
    net = models.__dict__[pr.net](args, pr, pertrained_model, device=device).to(device)
    criterion = models.__dict__[pr.loss](args, pr, device)
